...
- **p-value (setelah diff):** $8.67 \times 10^{-13}$
...
Tahap ini melanjutkan identifikasi ACF/PACF dengan pemodelan *Seasonal ARIMA*. Implementasi dilakukan menggunakan kelas `SARIMAX` pada pustaka `statsmodels`. Namun, karena tidak ada variabel *exogenous* ($X$) yang dimasukkan (`exog=None`), maka model yang dibangun secara konsep adalah **SARIMA** (SARIMAX tanpa komponen $X$).
